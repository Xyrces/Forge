...
| **P1** | skills loading into MAF agent instructions | ✅ done | `598b377` | `SqliteSkillSource` reads `agents/*.md` + the `skills` table, injects into `ChatClientAgent` instructions. |
...
| **P4 Stage A** | Checkpoint-based recovery on the in-process runtime | ✅ done | `d7233f4`–`6c21302` | Schema v11 adds `dispatch_checkpoint` (claimed → worktree_acquired → agent_completed → commit_done → push_done → pr_opened) + `checkpoint_at` + `recovery_attempts` + `recovery_report` table. Each engineering dispatch executor (`Worktree` / `RunAgent` / `CommitPushPr`) advances the checkpoint BEFORE its side-effect. `StartupRecovery` runs at startup, classifies every InProgress+assignee=forge issue, and replays unfinished side-effects (commit, push, PR open). `--recover` / `--recover-and-start` CLI flags + `POST /api/recovery/run` + `POST /api/recovery/dry-run` + `GET /api/recovery/reports` endpoints. Dashboard Recovery tab shows the latest report's actions. **3 kill-restart verification tests** prove the contract: commit→crash→push+PR, push→crash→PR, claim→crash→leave_alone. The previous P4 entry claimed an in-process reaper (`Spawner.StaleMinutes`); audit found nothing implemented. The new recovery service replaces it. |
| **P4 Stage B** | Durable execution via `Microsoft.Agents.AI.DurableTask` (opt-in) | ✅ done | `8443fef`, `467bc40` | Adds `Orchestrator:Execution=InProcess\|Durable` config switch + `IWorkflowDispatcher` abstraction. The workflow definition is unchanged — `ConfigureDurableWorkflows` registers the existing `EngineeringDispatchWorkflow` with a Durable Task Scheduler sidecar (DTS emulator image `mcr.microsoft.com/dts/dts-emulator:latest`, ports 8080/8082). `deploy/docker-compose.yml` brings the DTS sidecar up (works with both `docker compose` and `podman-compose`; vanilla OCI image). When `Execution=Durable`, the orchestrator's workflow state persists across crashes in the DTS; P4 Stage A's `StartupRecovery` becomes redundant for engineering-dispatch work but still helps the Designer / Artist / Groomer schedulers (which use fresh MAF agents per run, not durable workflows). Default remains `InProcess`. Webhook-driven PR merge signals were scoped to this stage but deferred to a follow-up — the existing 30s PRWatcher poll is fine for Stage B. |
...
| Post-P4 Stage A (checkpoint recovery) | 392 | 392 | 2 | `6c21302` |
| Post-P4 Stage B (durable dispatcher) | 392 | 392 | 2 | `467bc40` |
