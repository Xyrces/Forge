...
- **Failure triage (phases 1-3, schema v35).** `IssueStore.TransitionAsync` publishes `TaskFailureSignal` on failure-status boundary crossings (incl. metadata-stamped clearances — the `clearanceAction` + per-gesture `clearanceActionAt` nonce, phase-1 C4 edge); `FailureTriageConsumer` writes the `failure_triage` ledger (open → action → outcome, idempotent via store guards). Phase 2: a per-project opt-in flag (`$triage` in `project.roles_json`, `PUT /api/projects/{id}/triage`, the /triage banner toggle) gates a `TriageRequested` kick to `TriageConsumer` (own topic), which re-derives flag + ledger + the deterministic `TriageGuardrails` (≤2 triage actions/task/day, same-signature requeue twice without success = park) before running the `triage` role agent (`Agents/TriageAgent.cs`, prompt `agents/triage.md`, own AgentType/editable model). The agent's ONLY actions are `TriageTools`: requeue_with_guidance (spends a strike round — counters NOT reset), park_for_operator, flag_bug_suspect (ledger flag only, never creates issues), and (phase 3) **escalate_model** — writes a single-shot `llm/taskModel/<projectId>/<taskId>` marker and requeues the task; dispatch consumes the marker ONCE (no refund on run failure), runs the task on the role's configured **escalation model** (project escalation override → global escalation override → `llm.roles.<AgentType>.escalationModel` → unset; explicit-only — the tool refuses with an error string when the role has none), stamps `modelEscalated`/`modelEscalatedFrom`/`modelEscalatedTo` metadata, keys 429 cooldowns on the ESCALATED model, and draws concurrency from the `(project, escalated:<role>)` slot pool (cap 1, same SlotTable machinery) INSTEAD of the normal role pool. Escalation counts toward the ≤2/day/task triage cap. Every action audits with actor=triage on the ledger + `triageAction`/`triageNote` task metadata (TaskDetail strip).
...
- **Watch-lane QA stage (operator rule 2026-08-23, per-project `$qa` roles_json flag via `PUT /api/projects/{id}/qa`).** For opted projects every PR head gets a QA evaluation BEFORE the reviewer. The dispatcher first classifies the head's diff into an applicability tier (`Reviewer/QaEvidenceTierClassifier.cs`, deterministic — the agent never self-declares; stamped as `qaTier` metadata): **tier 1 visual** (any path under the visual prefixes — `$qa.visualPaths` override → default the project's clientdev `$territory` prefixes; unconfigured ⇒ nothing visual) runs full QA with raster PNG/JPG mandatory; **tier 2 code** (non-visual, non-docs; also empty/unclassifiable diffs — conservative) runs QA against the documented harness and a pass requires ≥1 evidence file of ANY type under `test-results/qa/<taskId>/` (raster not demanded; zero-evidence pass discarded as not-QA); **tier 3 docs** (every path is `docs/`, `**.md`, `.gitignore`/`.gitattributes`, `LICENSE*`, `test-results/` — `.github/` is deliberately code-tier) runs NOTHING and spends no attempt: the dispatcher stamps `qaVerdict=not-applicable` at the head, which the review relaunch and the merge gate treat like a pass. Runs use `Reviewer/QaDispatcher`'s dedicated QA worktree (`agent/qa-<taskId>`), the QA role runs with bash + no plan gate (`agents/qa.md` is the role card, the operational prompt is code-built and tier-aware), and the DISPATCHER — never the agent — ships the evidence DETACHED (operator decision 2026-08-25): force-added onto the dispatcher-owned `agent/qa-<taskId>` evidence branch (gitignored `test-results/` is expected — porthorizon's qa-1 policy — and harmless), force-pushed, and linked in a PR comment (rasters inline via raw URLs). The PR branch NEVER carries evidence (qaSha is always the code head; merged PRs leave no test-results/ churn on main); a non-ignored file outside the evidence paths refuses the ship. Verdict lands in task metadata (`qaForSha` code head verified, `qaSha` head the verdict applies to, `qaVerdict` pass|fail|not-applicable, `qaNotes`, `qaRound`, `qaTier`); the reviewer self-skips until QA is current (its `qaEnabled` guard), QA completion relaunches the review, the merge gate requires `pass` OR `not-applicable` at the current head, and a fail requeues a rework round with the QA notes (strike-counted). QA-infra failure budget: `QaDispatcher.MaxQaAttempts` (2) per head, then Blocked with `blockedKind=qa-unavailable` (operator-decision, no auto-resume).
...
- **Task type routes the role (operator rule 2026-08-23, after porthorizon task-752 hollow-closed).** Dispatch resolves the role from `task.Type` via `RoleAgentRegistry.FromTaskType`: `client`/`ui`/`godot`/`syncbridge` → clientdev, `test`/`playtest`/`qa` → the QA role, everything else → coredev. The groomer's `create_task` and the agents' `file_followup` (schema v36: `followup_draft.task_type`) carry the type through; mistyped tasks die at the plan-territory gate. A plan-gate-blocked run (planGate.failed=true) can never hollow-complete on a no-diff + NO_CHANGES_NEEDED message — it requeues as a failed attempt (CommitPushPrExecutor).
...
- **The Agents page is the agent control surface.** `/agents` shows per role: identity (name/type/territory/tools), the full role prompt (source: project override vs built-in), the effective provider+model (source: override | config | default), slot meter, the currently-running run (heartbeat: messages/tool calls/last-activity age) and the last finished run. The role catalog is canonical — `RoleAgentRegistry.All()` (engineering) + `RoleAgentRegistry.Pipeline` (scheduler-side) + `AllSlotRoles` (every pool) — so `/agents` and the project drill-down list the SAME 10 roles; pipeline model semantics are explicit (intake, triage, designer, groomer, and artist each have their own AgentType/editable model — NOTHING inherits anymore, phase 3 2026-08-23; orchestrator has no LLM). Model overrides are live and DB-backed (`Agents/RoleModelOverrides.cs`, memory keys `llm/roleModel/<AgentType>`; PUT/DELETE `/api/agents/roles/{name}/model`, incl. intake + triage) — the chat client factory and the run registry's model label consult them per run, no restart. Resolution order: override → `llm.roles` → provider default; a dangling override (provider removed) falls back to configured resolution. Each role also has an optional **escalation model** (phase 3; `llm/roleEscalationModel/<AgentType>` keys, PUT/DELETE `/api/agents/roles/{name}/escalation-model`, or `llm.roles.<AgentType>.escalationModel` config) — the triage agent's `escalate_model` target; unset means the role cannot be escalated. `MafAgentRunner` heartbeats `agent_run` after every model response (schema v21 `last_activity_at`) and persists the PARTIAL transcript on mid-run failure.
