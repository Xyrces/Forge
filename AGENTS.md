...
- **A sprint is a coherent, independently deployable unit (operator rule, 2026-07-27).** Two shapes only: ONE groomed spec-chain group, or ONE ad-hoc task. Ad-hoc tasks NEVER share a sprint — bundling unrelated parentless work mixed deployable states and bred intra-sprint conflicts (the "Ad-hoc work" grab-bag sprints are gone; each assembles solo, oldest first, sprint named after the task). The sole exception is blocker absorption (P1 groomed tasks joining the ACTIVE sprint to clear a merge-gate blocker).
