...
- **Watch-lane QA stage (operator rule 2026-08-23, per-project `$qa` roles_json flag via `PUT /api/projects/{id}/qa`).** For opted projects every PR head gets a QA playthrough BEFORE the reviewer: `Reviewer/QaDispatcher` syncs a dedicated QA worktree (`agent/qa-<taskId>`) to the head, runs the QA role (bash, no plan gate; `agents/qa.md` is the role card, the operational prompt is code-built), and the DISPATCHER — never the agent — commits+pushes the evidence to the PR branch, only when every touched path is under `test-results/` (a pass without raster PNG/JPG evidence is discarded as not-QA; JSON/SVG/ASCII state dumps are never screenshots). Verdict lands in task metadata (`qaForSha` code head verified, `qaSha` head the verdict applies to, `qaVerdict` pass|fail, `qaNotes`, `qaRound`); the reviewer self-skips until QA is current (its `qaEnabled` guard), QA completion relaunches the review, the merge gate additionally requires `qaVerdict=pass` at the current head, and a fail requeues a rework round with the QA notes (strike-counted). QA-infra failure budget: `QaDispatcher.MaxQaAttempts` (2) per head, then Blocked with `blockedKind=qa-unavailable` (operator-decision, no auto-resume).
