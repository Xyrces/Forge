...
- **A sprint is a coherent, independently deployable unit (operator rule, 2026-07-27).** Sprints assemble ONLY from groomed spec-chain groups — there are NO ad-hoc sprints of any size (the grab-bag "Ad-hoc work" sprints and the single-task variant are both gone). Ad-hoc (parentless) work enters a sprint only by **injection** into the ACTIVE sprint, when it belongs there: (1) its `followUpOf` chain reaches a sprint member (same work), (2) a `blocks` dep edge has it blocking a sprint member (enables/unblocks the ongoing work — the merge-gate harness-fix case), (3) operator P1 / `blocker=true` flag, or (4) operator requeue (`requeuedFromFailedAt` — otherwise requeued ad-hoc work would strand). Unrelated groomed ad-hoc tasks wait in the backlog; the operator promotes them through intake as an epic when they should run.
