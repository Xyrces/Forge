...
- **LLM 429 cooldowns are per (provider, model), not global.** `Orchestrator/ModelRateLimitTracker.cs` keys cooldowns by the resolved provider+model (via `LlmConfig.Resolve(FromTaskType(task.Type))`) — a minimax 429 must not freeze tasks pinned to another model (e.g. kimi-k3 for grooming/review/intake). Missing/broken LLM config degrades to a single "default" bucket (the old global behavior).
