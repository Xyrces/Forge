...
public sealed class TempRoot : IDisposable
{
    ...
    private TempRoot()
    {
        // Capture the process CWD before we create our temp root so we can
        // restore it safely during cleanup instead of leaving the process with a
        // deleted directory as CWD.
    ...
    public IssueStore NewIssueStore(string prefix)
    {
        ...
        return new IssueStore(BuildConnectionString(path));
    }
    ...
    public void Dispose()
    {
        ...
        // current working directory. If the CWD is inside the temp root we
        // first try to restore the original CWD captured at construction, then
        // fall back to the worktree root. If we cannot move the CWD out of the
        // temp root, we leave the directory behind rather than deleting it from
        // under the process, which would cause spawned git/IO children to fail
        // with "Unable to read current working directory: No such file or directory".
        try
        {
            var cwd = Directory.GetCurrentDirectory();
            var rootFull = Path.GetFullPath(_root);
            var cwdFull = Path.GetFullPath(cwd);
            var cwdUnderRoot = cwdFull.Equals(rootFull, StringComparison.OrdinalIgnoreCase)
                || cwdFull.StartsWith(rootFull + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase)
                || cwdFull.StartsWith(rootFull + Path.AltDirectorySeparatorChar, StringComparison.OrdinalIgnoreCase);

            if (cwdUnderRoot)
            {
                // Try the original CWD first; if it no longer exists, fall back to the worktree root.
                var safeCwd = _originalCwd;
                if (!Directory.Exists(safeCwd))
                {
                    safeCwd = GetWorktreeRoot();
                }

                if (Directory.Exists(safeCwd))
                {
                    Directory.SetCurrentDirectory(safeCwd);
                }
            }
        }
        catch { }

        // Only delete the root if the process CWD is no longer inside it.
        try
        {
            var cwdAfter = Directory.GetCurrentDirectory();
            var rootFull = Path.GetFullPath(_root);
            var cwdAfterFull = Path.GetFullPath(cwdAfter);
            var stillUnderRoot = cwdAfterFull.Equals(rootFull, StringComparison.OrdinalIgnoreCase)
                || cwdAfterFull.StartsWith(rootFull + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase)
                || cwdAfterFull.StartsWith(rootFull + Path.AltDirectorySeparatorChar, StringComparison.OrdinalIgnoreCase);
